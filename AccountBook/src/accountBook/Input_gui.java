...
public class Input_gui extends JFrame{
	...
	public Input_gui(String id, String setY, String setM, String setD, String setMoney, String setUsing, String setInOrout, String key){
		this.id = id;
		this.setY = setY;
		this.setM = setM;
		this.setD = setD;
	...
	public void setInput_gui(){
		...
		if(setInOrout == "ÁöÃâ"){outCh.setSelected(true);}
		...
		input.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
			    ...
			    try {
					if(key == null){flag = in.account(y,m,d,inOrout,money,using);}
					else{
						String queary ="update calender_info_"+id+" set date='"+y+m+d+"', inOrout='"+inOrout+"', money="+money+", p_use='"+using+"' where cnt="+key;
						ip.productUpdateDelete(queary);
						int balance= Integer.valueOf(ip.productSelect("getBalance","select balance from user_info where id="+id));
						if(inOrout.equals("ÁöÃâ")){
							System.out.println("ÀÜ°íÁöÃâÀúÀå");
							balance -= money;	
						}else if(inOrout.equals("¼öÀÔ")){
							System.out.println("ÀÜ°í¼öÀÔÀúÀå");
							balance += money;
						}
						//ÀÜ°í ¾÷µ¥ÀÌÆ® 
			    ...
			    System.out.println("input_gui flag action ³»ºÎ = "+flag);
			    dispose();
			  }
		});
